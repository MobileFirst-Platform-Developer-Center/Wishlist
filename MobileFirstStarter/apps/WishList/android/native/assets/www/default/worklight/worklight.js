...
__WLClient = function() {
	...
	this.logout = function(realm, options) {
		...
		function onLogoutSuccess(transport) {
			if ( typeof userInfo[realm] === "undefined") {
				WL.Logger.error('onLogoutSuccess: realm: ' + realm + ' is not part of userInfo object.');
            } else {
		...
		WLAuthorizationManager.__getClientId().then (
			function (clientId) {
				if (typeof(clientId) !== 'undefined' && clientId !== null) {
					...
					WL.Logger.error("Cannot logout before client is registered.");
					return;
				}
			}, 
			function () {
				WL.Logger.error("Failed to get client id");
				return;
			}
		);
	};
